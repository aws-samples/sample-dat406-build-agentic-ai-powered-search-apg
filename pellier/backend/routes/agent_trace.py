...
async def _audit_row_by_id(audit_id: int | None) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.warning("Agent Trace audit row lookup unavailable: %s", exc)
        return None
...
async def _latest_governed_receipt(
    *,
    ...
    try:
        ...
        logger.debug("Agent Trace governed receipt unavailable: %s", exc)
        return None
...
async def _collect_readiness() -> dict[str, Any]:
    ...
    checks.append(_readiness_check(
        ...
        detail=(
            "User pool, app client, and test credential secret configured for JWT passthrough."  # copy-allow: agent-trace-readiness-detail
            if cognito_ready
...
async def _collect_proof_board(session_id: str | None = None) -> dict[str, Any]:
    ...
    cards = [
        ...
        },
        {
            ...
            "lab": "Lab 2: Choose the Search Strategy to Ship",
            ...
            "fallback": {
                "label": "Terminal fallback",
                "command": (
                    "curl -s 'http://localhost:8000/api/agent-trace/search-strategies/compare"
