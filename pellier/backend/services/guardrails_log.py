...
A small per-session ring buffer of guardrail outcomes so Pellier Labs
...
Populated from two places:
  ...
  1. ``routes/chat.py`` / ``services/chat.py`` — when a turn invokes
     ...
     is recorded here so Pellier Labs Guardrails lane shows live status.
