...
# The canonical Lab 2 query and its labeled relevant catalog rows. Golden ids
# are the in-stock Home Decor pieces tagged both ``gift`` and ``home`` at or
# under $100 in ``scripts/seed_pellier_catalog.py``. The eval harness pins the
# same entry; ``tests/test_search_micro_eval.py`` keeps the two aligned.
CANONICAL_ANNA_QUERY = "A housewarming gift under $100 that is currently in stock."
CANONICAL_ANNA_GOLDEN_IDS = ("21", "22", "23", "25", "27", "29")
