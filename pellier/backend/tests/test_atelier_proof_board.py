"""Tests for the Atelier Proof Board and readiness API."""
...
from routes import atelier_observatory
from routes.atelier_observatory import router as atelier_router
...
def _client(stub_db: _ProofDB) -> TestClient:
    ...
    fast = FastAPI()
    fast.include_router(atelier_router)
...
def test_readiness_reports_live_pillars(monkeypatch) -> None:
    ...
    r = client.get("/api/atelier/readiness")
...
def test_proof_board_returns_cards_receipt_and_fallbacks(monkeypatch) -> None:
    ...
    r = client.get("/api/atelier/proof-board?session_id=managed-proof")
    assert r.status_code == 200
    body = r.json()
