...
For every ``(live_path, solution_path)`` pair:
  ...
  7. The inventory solution keeps the ``product_query`` signature and
...
def test_live_file_has_workshop_markers(
    ...
    Pellier Labs' Code Editor won't know where to focus.
    """
...
def test_floor_check_builder_contract() -> None:
    ...
    real verification of a correct wire is Pellier Labs Tools strip flipping
    ...
    if "floor_check is in stub state" not in live_src:
        pytest.skip(
            ...
            "wire via Pellier Labs Tools 15/15 strip and Marco's Brooklyn turn."
        )
