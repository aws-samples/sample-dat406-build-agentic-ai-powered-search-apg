...
def test_proof_board_returns_cards_receipt_and_fallbacks(monkeypatch) -> None:
    ...
    assert cards["retrieval-comparison"]["lab"] == "Lab 2: Choose the Search Strategy to Ship"
    assert cards["retrieval-comparison"]["status"] == "available"
    assert cards["managed-rail"]["lab"] == "Lab 3: Take the Agent from Local to Managed"
    assert cards["managed-rail"]["required"] is True
    assert cards["audit-ledger"]["lab"] == "Lab 3: Take the Agent from Local to Managed"
