/**
 * Unit tests for the Observatory data layer hooks.
 *
 * Tests verify that useObservatoryData correctly loads fixture data,
...
describe('useObservatoryData', () => {
  it('returns fixture data when source is "fixture"', async () => {
    const { result } = renderHook(() =>
      useObservatoryData({ key: 'sessions', source: 'fixture' }),
    );
  ...
  it('sets loading state during fetch', async () => {
    const { result } = renderHook(() =>
      useObservatoryData({ key: 'agents', source: 'fixture' }),
    );
  ...
  it('sets error state when fixture key is invalid', async () => {
    const { result } = renderHook(() =>
      useObservatoryData({ key: 'nonexistent-fixture-key', source: 'fixture' }),
    );
  ...
  it('returns data with refetch function', async () => {
    const { result } = renderHook(() =>
      useObservatoryData({ key: 'tools', source: 'fixture' }),
    );
...
describe('useToolDiscovery', () => {
  ...
  it('calls POST /api/observatory/tools/discover with correct body', async () => {
    const mockResponse = {
      results: [
        {
          rank: 1,
    ...
    expect(url).toBe('/api/observatory/tools/discover');
