 ...
 *
 * Generates valid Observatory route paths from the defined route segments
...
// ---------------------------------------------------------------------------
const observatoryRoutes: RouteObject[] = [
  {
    path: '/observatory',
...
// ---------------------------------------------------------------------------
// Generators — produce valid Observatory paths from defined route segments.
// ---------------------------------------------------------------------------
...
/**
 * Generates a valid Observatory route path. The generator picks from all
 * defined route segments, including parameterized routes with generated
 * parameter values.
 */
const observatoryPathArb: fc.Arbitrary<string> = fc.oneof(
  // Static leaf routes
  fc.constant('/observatory'),
  fc.constant('/observatory/proof-board'),
  fc.constant('/observatory/sessions'),
  fc.constant('/observatory/architecture'),
  fc.constant('/observatory/agents'),
  fc.constant('/observatory/skills'),
  fc.constant('/observatory/tools'),
  fc.constant('/observatory/search'),
  fc.constant('/observatory/routing'),
  fc.constant('/observatory/memory'),
  fc.constant('/observatory/write-path'),
  fc.constant('/observatory/performance'),
  ...
  paramValueArb.map((id) => `/observatory/sessions/${id}`),

  // Parameterized: sessions/:id/:tab
  fc.tuple(paramValueArb, sessionTabArb).map(
    ([id, tab]) => `/observatory/sessions/${id}/${tab}`,
  ),

  // Parameterized: architecture/:concept
  conceptSlugArb.map((concept) => `/observatory/architecture/${concept}`),
)
...
describe('Property 5: Route resolution correctness', () => {
  it('every valid Observatory path resolves to a non-null route match', () => {
    fc.assert(
      fc.property(observatoryPathArb, (path) => {
        const matches = matchRoutes(observatoryRoutes, path)

        // matchRoutes returns null when no route matches the path.
        // Every valid Observatory path must produce at least one match.
