 ...
 * in the Observatory fixture set, loading the fixture via useObservatoryData with
 ...
 * no values mutated.
 *
 * Coverage is derived from useObservatoryData's own FIXTURE_KEYS export, so a
...
import { useObservatoryData, FIXTURE_KEYS } from '../hooks/useObservatoryData';
 ...
 * sync with useObservatoryData's fixtureImporters — the guard test below
...
describe('Property 4: Fixture data round-trip integrity', () => {
  it.each(fixtureEntries)(
    'useObservatoryData("%s", source: "fixture") returns data identical to raw JSON import',
    async (key, rawJson) => {
      const { result } = renderHook(() =>
        useObservatoryData({ key, source: 'fixture' }),
      );
  ...
  it('covers every fixture key registered in useObservatoryData (no silent gaps)', () => {
