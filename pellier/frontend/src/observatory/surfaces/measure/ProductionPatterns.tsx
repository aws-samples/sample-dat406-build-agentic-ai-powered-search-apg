 ...
 * three new patterns that don't have a home elsewhere in the Observatory.
 */
...
const CATEGORY_TONE: Record<ProductionPattern['category'], { label: string; color: string }> = {
  auth: { label: 'Auth', color: 'var(--obs-red-1)' },
  policy: { label: 'Policy', color: 'var(--obs-green-1)' },
  ops: { label: 'Ops', color: '#7c6f64' },
  scaling: { label: 'Scaling', color: 'var(--obs-red-1)' },
};
    ...
    <div style={{ marginTop: '18px' }}>
      ...
      >
        ...
        >
          ...
          <code style={{ fontFamily: 'var(--obs-mono)', fontSize: '13px', color: 'var(--obs-ink-1)' }}>
...
const ProductionPatterns: React.FC = () => {
  const { data, loading, error, refetch } = useObservatoryData<ProductionPatternsData>({
