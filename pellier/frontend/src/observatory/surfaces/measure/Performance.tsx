      ...
      <div style={{ marginTop: '16px', overflowX: 'auto' }}>
        ...
        >
          {histogram.map((bucket, i) => {
            ...
            const fill = bucket.type === 'warm' ? 'var(--obs-green-1)' : 'var(--obs-red-1)';
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <span
            style={{
              ...
              height: '10px',
              borderRadius: '2px',
              backgroundColor: 'var(--obs-green-1)',
              opacity: 0.8,
    ...
    <ExpCard>
      ...
      <div style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {budget.map((row) => {
          ...
          const color = TYPE_COLORS[row.type] ?? 'var(--obs-ink-4)';
          ...
          return (
            <div key={row.panel} style={{ display: 'grid', gridTemplateColumns: '200px 1fr 116px', gap: '12px', alignItems: 'center' }}>
              ...
              >
                {row.p50Ms}ms
                <span style={{ color: 'var(--obs-ink-3)', marginLeft: '4px', fontSize: '13px' }}>
                  / {row.maxMs}ms
                </span>
              </span>
            </div>
 ...
 *      we hit /api/observatory/search-strategies/compare which executes all
...
const ExtractedFiltersStrip: React.FC<ExtractedFiltersStripProps> = ({
  ...
  const usedColor = used.tone === 'green' ? 'var(--obs-green-1)' : 'var(--obs-red-1)';
  ...
  return (
    <div
      style={{
        ...
        gap: '8px',
        padding: '10px 12px',
        backgroundColor: 'color-mix(in srgb, var(--obs-green-1) 4%, transparent)',
        border: '1px solid var(--obs-card-border)',
        borderRadius: '6px',
      }}
    >
...
const SearchStrategyComparison: React.FC<SearchStrategyComparisonProps> = ({
  ...
  const handleRun = async () => {
    ...
    try {
      const r = await fetch(
        `/api/observatory/search-strategies/compare?query=${encodeURIComponent(q)}`,
      );
      if (!r.ok) {
    ...
    <ExpCard>
                  ...
                  <tr style={{ backgroundColor: rowBg }}>
                    <td
                      style={{
                        ...
                        color: isShipped ? 'var(--obs-ink-1)' : cellStyle.color,
                      }}
                    >
                      {s.strategy}
                      {s.rerank && (
      ...
      >
        ...
        >
          ...
          stems together - exactly the shape of query a storefront shopper asks.
...
const StorageUsageBars: React.FC<StorageUsageProps> = ({ usage }) => {
  const barColors = ['var(--obs-red-1)', 'var(--obs-green-1)', '#7c6f64'];
        ...
        {TIME_WINDOWS.map((w) => (
          <button
            ...
            style={{
              ...
              padding: '5px 14px',
              borderRadius: '100px',
              border: activeWindow === w ? '1px solid var(--obs-ink-1)' : '1px solid var(--obs-card-border)',
              backgroundColor: activeWindow === w ? 'var(--obs-ink-1)' : 'transparent',
              color: activeWindow === w ? 'var(--obs-cream-1)' : 'var(--obs-ink-1)',
              cursor: 'pointer',
...
const Performance: React.FC = () => {
  const { data, loading, error, refetch } = useObservatoryData<PerformanceData>({
    key: 'performance',
  });
