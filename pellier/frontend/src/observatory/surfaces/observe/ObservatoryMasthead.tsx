...
import './ObservatoryMasthead.css';
...
export interface ObservatoryMastheadProps {
  eyebrow?: string;
  title?: string;
  deck?: string;
  status?: string;
...
export function ObservatoryMasthead({
  ...
  return (
    <header className="observatory-masthead">
      <div className="observatory-masthead-copy">
        <Eyebrow label={eyebrow} />
        <h1 className="font-display">{title}</h1>
        <p>{deck}</p>
        {status ? (
          <span className="observatory-masthead-status">
            <span aria-hidden="true" />
            {status}
          </span>
        ) : null}
      </div>
      <div className="observatory-masthead-flow" aria-hidden="true">
        <canvas ref={canvasRef} data-testid="observatory-particle-canvas" />
      </div>
    </header>
  );
}
