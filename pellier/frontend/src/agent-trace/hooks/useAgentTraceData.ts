 ...
 *
 * Abstracts fixture vs. API data loading for all Agent Trace surfaces.
 * In fixture mode (default), dynamically imports from /fixtures/ based on key.
 * In API mode, fetches from /api/agent-trace/* endpoints with transparent fallback
...
export interface UseAgentTraceDataOptions {
  key: string;
  params?: Record<string, string>;
  source?: DataSource;
}

export interface UseAgentTraceDataResult<T> {
  data: T | null;
  loading: boolean;
  error: string | null;
  refetch: () => void;
 ...
 * route); the surface is a teaching catalog and `useAgentTraceData` falls back
 * to the fixture loader when the key is absent here.
 */
const apiEndpoints: Record<string, string> = {
  sessions: '/api/agent-trace/sessions',
  agents: '/api/agent-trace/agents',
  tools: '/api/agent-trace/tools/list',
  routing: '/api/agent-trace/routing',
  skills: '/api/agent-trace/skills',
  performance: '/api/agent-trace/performance',
  evaluations: '/api/agent-trace/evaluations',
  observatory: '/api/agent-trace/observatory',
  architecture: '/api/agent-trace/architecture',
};
...
function buildApiUrl(key: string, params?: Record<string, string>): string {
  ...
  if (key.startsWith('session-') && key !== 'sessions') {
    ...
    return `/api/agent-trace/sessions/${id}`;
  }
  if (key.startsWith('memory-')) {
    const persona = key.replace('memory-', '');
    return `/api/agent-trace/memory/${persona}`;
  }

  const base = apiEndpoints[key];
  if (!base) return `/api/agent-trace/${key}`;
...
export function useAgentTraceData<T = unknown>(
  options: UseAgentTraceDataOptions,
): UseAgentTraceDataResult<T> {
