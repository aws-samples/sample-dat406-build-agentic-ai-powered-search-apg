 ...
 * no values mutated.
 *
 * Coverage is derived from useAgentTraceData's own FIXTURE_KEYS export, so a
...
import { useAgentTraceData, FIXTURE_KEYS } from '../hooks/useAgentTraceData';
 ...
 * sync with useAgentTraceData's fixtureImporters — the guard test below
...
describe('Property 4: Fixture data round-trip integrity', () => {
  it.each(fixtureEntries)(
    'useAgentTraceData("%s", source: "fixture") returns data identical to raw JSON import',
    async (key, rawJson) => {
      const { result } = renderHook(() =>
        useAgentTraceData({ key, source: 'fixture' }),
      );
  ...
  it('covers every fixture key registered in useAgentTraceData (no silent gaps)', () => {
