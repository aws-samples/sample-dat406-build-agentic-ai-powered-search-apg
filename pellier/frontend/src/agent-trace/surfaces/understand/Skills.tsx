...
const SkillRouterDemoCard: React.FC<SkillRouterDemoCardProps> = ({
  ...
  const run = useCallback(
    async (q: string) => {
      ...
      try {
        const r = await fetch('/api/agent-trace/skills/route', {
          method: 'POST',
...
const SkillCard: React.FC<SkillCardProps> = ({
  ...
  <ExpCard>
    ...
    {/* Description — sans Instrument Sans, mirroring the AgentTraceWelcome
...
const Skills: React.FC = () => {
  const { data, loading, error, refetch } = useAgentTraceData<Skill[]>({ key: 'skills' });
  const skills = data ?? [];
