...
describe('receipt links', () => {
  ...
  it('routes a known session to its telemetry evidence', () => {
    ...
    expect(route).toBe('/atelier/sessions/sess-1/telemetry')
  })
  ...
  it('falls back to a real evidence route when the session is unknown', () => {
    ...
    expect(receiptRoute({})).toBe('/atelier/audit-proof')
    expect(receiptRoute({ sessionId: null })).toBe('/atelier/audit-proof')
  })
