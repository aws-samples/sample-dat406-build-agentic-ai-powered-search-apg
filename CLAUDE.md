...
- Agent Trace → **UNDERSTAND → Tools** strip flips from **14/15** to **15/15 shipped**.
...
stubbed and skips once it is wired correctly** (never fails — see the test's own
POLARITY NOTE). Either way its result says nothing about whether the wire works,
so it is not a verify step. The two checks above are the real verification.
