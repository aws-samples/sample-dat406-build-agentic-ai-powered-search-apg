...
Tool and agent naming follows `coding-standards.md` and `workshop-content.md` steering. Model construction follows `coding-standards.md`: Sonnet 5 and Opus 5 are used without temperature overrides.
...
- **Architecture diagram** (Mermaid): browser → Cognito Hosted UI → FastAPI → (AgentCore Identity → AgentCore Memory → AgentCore Runtime → AgentCore Gateway → Aurora pgvector → Bedrock: Opus 5 + Sonnet 5 + Cohere Embed v4 + Cohere Rerank v3.5)
- **Sequence diagrams:**
...
- [ ] Specialists use Claude Opus 5 with no temperature override per `coding-standards.md`
